...
public class CustomIntegerArrayList {
		...
		public int remove(int index) {
			int removedElement = this.arr.get(index);
		...
		public void remove(int num, int element) {
			...
			for (int i = 0;i<size;i++) {
				if (element == this.arr.get(aux) && (cont < num)){
					arr.remove(aux);
					cont += 1;
				}else {
					aux += 1;
		...
		public ArrayList<Integer> splice(int index, int num) {
			...
			if ((index < 0) || (index >= size) || ( num == 0)){
				return returnArrayList;
			}else {			
				int cont = 0;
				for (int i = 0;i<size;i++) {
					if ((i >= index) && (cont < num)){
						returnArrayList.add(this.arr.get(index));
						this.arr.remove(index);
						cont += 1;
					}
				}
		...
		public ArrayList<Integer> splice(int index, int num, int[] otherArray) {
			...
			ArrayList<Integer> returnArrayList = this.splice(index, num);
			if (returnArrayList.size() == 0){
				return returnArrayList;
			}else {
				for (int i = otherArray.length-1;i>=0;i--) {
					this.arr.add(index, otherArray[i]);
				}
